...
export function useInputHandlers(ctx) {
  ...
  const precisionWheelRef = useRef({
    active: false,
    dir: 0,
    lastEventAtMs: 0,
    lastScrollAtMs: 0
  ...
  useInput((ch, key) => {
    ...
    if (key.wheelUp || key.wheelDown) {
      ...
      if (hasModifier || precisionSticky) {
        ...
        if (dir_1 === precision.dir && now - precision.lastScrollAtMs < PRECISION_WHEEL_MIN_GAP_MS) {
          return;
        }
        precision.lastScrollAtMs = now;
        precision.dir = dir_1;
        return scrollTranscript(dir_1 * wheelStep);
      }
      precision.active = false;
      // 0 = direction-flip bounce deferred; skip the no-op scroll.
      const rows = computeWheelStep(wheelAccelRef.current, dir_1, now);
