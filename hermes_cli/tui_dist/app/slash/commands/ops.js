...
export const opsCommands = [{
  ...
  name: 'stop',
  run: (_arg, ctx) => {
    ctx.gateway.rpc('process.stop', {}).then(ctx.guarded(r => {
  ...
  run: (arg, ctx) => {
    ...
    ctx.gateway.rpc('reload.mcp', params).then(ctx.guarded(r => {
      if (r.status === 'confirm_required') {
        ctx.transcript.sys(r.message || '/reload-mcp requires confirmation');
        return;
      }
      if (r.status === 'reloaded') {
        ctx.transcript.sys(params.always ? 'MCP servers reloaded · future /reload-mcp will run without confirmation' : 'MCP servers reloaded');
        return;
      }
  ...
  name: 'reload',
  run: (_arg, ctx) => {
    ctx.gateway.rpc('reload.env', {}).then(ctx.guarded(r => {
  ...
  run: (arg, ctx) => {
    ...
    ctx.gateway.rpc('browser.manage', {
      action,
      session_id: sid,
      ...(url && {
        url
      ...
      if (action === 'status') {
        return ctx.transcript.sys(r.connected ? `browser connected: ${r.url || '(url unavailable)'}` : 'browser not connected (try /browser connect <url> or set browser.cdp_url in config.yaml)');
      }
  ...
  run: (arg, ctx) => {
    ...
    if (!trimmed || lower === 'list' || lower === 'ls') {
      return ctx.gateway.rpc('rollback.list', {
        session_id: ctx.sid
      }).then(ctx.guarded(r => {
        if (!r.enabled) {
        ...
        ctx.transcript.panel('Rollback checkpoints', [{
          rows: checkpoints.map((c, idx) => [`${idx + 1}. ${c.hash.slice(0, 10)}`, [c.timestamp, c.message].filter(Boolean).join(' · ') || '(no metadata)'])
        }]);
      })).catch(ctx.guardedErr);
    }
    if (lower === 'diff') {
      ...
      return ctx.gateway.rpc('rollback.diff', {
        hash,
        session_id: ctx.sid
      }).then(ctx.guarded(r => {
    ...
    return ctx.gateway.rpc('rollback.restore', {
      ...(filePath ? {
        file_path: filePath
      } : {}),
      hash,
  ...
  run: (arg, ctx, cmd) => {
    ...
    if (subcommand !== 'disable' && subcommand !== 'enable') {
      ctx.gateway.gw.request('slash.exec', {
        command: cmd.slice(1),
        session_id: ctx.sid
      }).then(r => {
        if (ctx.stale()) {
