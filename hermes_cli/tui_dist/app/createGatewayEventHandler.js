...
export function createGatewayEventHandler(ctx) {
  ...
  const scheduleThinkingStatus = status => {
    ...
    thinkingStatusTimer = setTimeout(() => {
      thinkingStatusTimer = null;
      patchUiState({
        status: pendingThinkingStatus || statusFromBusy()
      });
    }, STREAM_BATCH_MS);
  };
  ...
  return ev => {
    ...
    switch (ev.type) {
      ...
      case 'session.info':
        {
          ...
          setHistoryItems(prev => prev.map(m => m.kind === 'intro' ? {
            ...m,
            info
          } : m));
          return;
      ...
      case 'gateway.start_timeout':
        {
          ...
          const tailLines = (stderrTail ?? '').split('\n').map(l => l.trim()).filter(Boolean).slice(-STDERR_LINES_MAX);
      ...
      case 'subagent.spawn_requested':
        ...
        turnController.upsertSubagent(ev.payload, c => isTerminalStatus(c.status) ? {} : {
          status: 'queued'
        });
