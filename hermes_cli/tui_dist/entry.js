...
gw.start();
const dumpNotice = (snap, dump) => `hermes-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`;
setupGracefulExit({
  cleanups: [() => {
    resetTerminalModes();
    return gw.kill();
...
const [ink, {
  ...
  trackFrame
}] = await Promise.all([import('@hermes/ink'), import('./app.js'), import('./lib/perfPane.js'), import('./lib/fpsStore.js')]);
