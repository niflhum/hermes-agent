...
export function useVirtualHistory(scrollRef, items, columns, {
  ...
  if (offsetsCache.current.version !== offsetVersion.current || offsetsCache.current.n !== n) {
    ...
    for (let i_0 = 0; i_0 < n; i_0++) {
      arr[i_0 + 1] = arr[i_0] + ensureVirtualItemHeight(heights.current, items[i_0].key, i_0, estimate, estimateHeight);
    }
  ...
  if (n > 0 && vp > 0 && !frozenRange) {
    ...
    for (let i_1 = start; i_1 < end; i_1++) {
      coverage += ensureVirtualItemHeight(heights.current, items[i_1].key, i_1, PESSIMISTIC, estimateHeight);
    }
    if (sticky) {
  ...
  const measureRef = useCallback(key => {
    ...
    if (!fn) {
      fn = el => {
        ...
        const h_0 = Math.ceil(existing?.yogaNode?.getComputedHeight?.() ?? 0);
  ...
  useLayoutEffect(() => {
    ...
    if (s_0 && shouldSetVirtualClamp({
      ...
      s_0.setClampBounds(clampMin, clampMax);
    } else {
    ...
    if (skipMeasurement.current) {
      ...
      for (let i_2 = effStart; i_2 < effEnd; i_2++) {
        ...
        const h_1 = Math.ceil(nodes.current.get(k_1)?.yogaNode?.getComputedHeight?.() ?? 0);
    ...
    if (s_0) {
      const next = {
        sticky: s_0.isSticky(),
        top: Math.max(0, s_0.getScrollTop() + s_0.getPendingDelta()),
        vp: Math.max(0, s_0.getViewportHeight())
      };
